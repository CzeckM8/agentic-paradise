...
func _build_sprite_grid():
	...
	for key in sheet_order:
		...
		# Small single-frame PNGs use the whole image; larger sheets use top-left frame.
		var preview_tex: Texture2D
		if tex is Texture2D and tex.get_width() <= 48 and tex.get_height() <= 48:
			preview_tex = tex
		else:
			var atlas = AtlasTexture.new()
			atlas.atlas = tex
			atlas.region = Rect2(0, 0, min(FRAME_SIZE, tex.get_width()), min(FRAME_SIZE, tex.get_height()))
			preview_tex = atlas
